...
#define MENSAJE_PAUSA "Presione Enter para continuar"
...
int inputContinuar(char message[]);
...
/** \brief hace una pausa para que el usuario presione Enter
 *
 * \param message[] char Es el mensaje a ser mostrado.
 *
 */
void pausa(char message[]);

/** \brief convierte una cadena de texto a entero
 *
 * \param inputStr[] char cadena de entrada
 * \param ouputInt int* valor convertido
 * \return int retorna 0 si hubo error 1 si esta ok
 *
 */
int inputStringToInt(char inputStr[], int* ouputInt);
